...
LAMBDA_EXP = .5                     # average time between customer demands (months)
# DEMAND_SIZES = [1, 2, 3, 4]         # possible customer demand sizes
# DEMAND_PROB = [1/6, 1/3, 1/3, 1/6]  # probability of each demand size
DEMAND_SIZES = [1, 2, 3, 4]         # possible customer demand sizes
DEMAND_PROB = [0.3, 0.4, 0.2,0.1]  # probability of each demand size
START_INVENTORY = 70.               # units in inventory at simulation start
COST_ORDER_SETUP = 10.              # fixed cost of placing an order
COST_ORDER_PER_ITEM = 50.            # variable cost of ordering an item
...
T = 24.                   # duration of the simulation (in months)
...
class InventorySystem:
    ...
    def place_order(self, env, units):
        ...
        lead_time = np.random.normal(.5, 1.0)
    ...
    def demands(self, env):
        ...
        while True:
            ...
            iat = np.random.exponential(LAMBDA_EXP)
...
def run(reorder_point:float, order_size:float, display_chart = False):
    ...
    avg_total_cost = (inv.ordering_cost 
                    + inv.holding_cost 
                    + inv.shortage_cost) / T
    avg_ordering_cost = inv.ordering_cost / T
    avg_holding_cost = inv.holding_cost / T
    avg_shortage_cost = inv.shortage_cost / T
...
if __name__ == '__main__':
    clear()
    #        def run(reorder_point:float, order_size:float, display_chart = False):
    simulation = run(20,10,display_chart=True)
